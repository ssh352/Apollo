...
def get_momentum_list(mom_list):
    ...
    for mom in mom_list:
        ...
        result = w.wsi(mom, "close", date_now.strftime('%Y-%m-%d')+" 11:00:00", date_now.strftime('%Y-%m-%d')+" 11:30:00", "BarSize=30")
...
def get_trade_item(trade_list, flag):
    ...
    if flag == 1:
        if os.path.exists("long_N.csv"):
            with open("long_N.csv", "r+") as csvfile:
            ...
            if (date_now_t - date_pre_t).days >= 5:
                with open("long_N.csv", "ab+") as csvfile:
    ...
    if flag == 0:
        if os.path.exists("short_N.csv"):
            with open("short_N.csv", "r+") as csvfile:
                lines = csv.reader(csvfile)
                for line in lines:
                    date_pre = line[0]
            ...
            if (date_now_t - date_pre_t).days >= 5:
                with open("short_N.csv", "ab+") as csvfile:
                    writer = csv.writer(csvfile)
                    writer.writerow(list_main)
        else:
            with open("short_N.csv", "ab+") as csvfile:
