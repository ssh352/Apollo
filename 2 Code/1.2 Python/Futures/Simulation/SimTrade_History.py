...
# 根据动量排名获取多空组合
def get_momentum_list(date_now, mom_list):
    ...
    # dict_sort = sorted(dict_item.items(), key=lambda item: item[1], reverse=True)
...
# 根据展期收益率排名获取多空组合
def get_extension_list(date_now, ext_list, flag):
...
# 输出交易品种记录
def get_trade_item(date_now, trade_list, flag):
...
# 输出具体数值到txt文件
def write_ext_txt(date_now, long_item, short_item):
    list_main_long = []
    list_main_short = []
    ...
    list_main_long.insert(0, date_now)
    list_main_short.insert(0, date_now)
...
if __name__ == "__main__":
    ...
    for day in list_step_day:
        ...
        list_valid = get_valid_list(day)

        long_list, short_list = get_momentum_list(day, list_valid)

        long_key = dict(long_list).keys()
        short_key = dict(short_list).keys()

        print "long_key:", long_key
        print "short_key:", short_key

        long_item = get_extension_list(day, long_key, 1)
        short_item = get_extension_list(day, short_key, 0)

        print "long_item:", long_item
        print "short_item:", short_item

        # dict_long_item = dict(long_item)
        # sort_long_item = sorted(dict_long_item.items(), key=lambda item:item[0])

        write_ext_txt(day, long_item, short_item)

        get_trade_item(day, dict(long_item).keys(), 1)
        get_trade_item(day, dict(short_item).keys(), 0)
