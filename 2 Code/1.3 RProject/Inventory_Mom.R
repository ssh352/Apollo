...
for(i in seq(KReserve+1,(length(trade_day)-KHolding), KHolding))
{
  if(trade_day[i]>(StartDay+NPeriod))
  {
    ...
    Mom_sort<-matrix(NA,nrow =length(index_mom),ncol = 2)
    Mom_sort[,1]<-index_mom
    Mom_sort[,2]<-Mom_Tmp[index_mom,2]
    ...
    bench_re_Max_Long <- as.matrix(Matrix_Return[(i+1):(i+KHolding),Mom_long_index])
    bench_re_Max_Long[which(is.na(bench_re_Max_Long)==1,arr.ind = T)] <- 0
    
    bench_re_Max_Short <- as.matrix(Matrix_Return[(i+1):(i+KHolding),Mom_short_index])
