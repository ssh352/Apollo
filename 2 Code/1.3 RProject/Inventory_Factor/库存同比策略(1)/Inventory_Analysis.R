...
for(i in seq(1,length(trade_day), 20))
{
...
plot(cumsum(quantile_max[500:length(quantile_max)]),type="l")

plot(cumsum(quantile_max[500:nrow(quantile_max),5]-quantile_max[500:nrow(quantile_max),1]),type="l",ylim = c(-2.0,2), xaxt="n")
axis(1,at=500:length(trade_day[500:length(trade_day)]),labels=trade_day[500:length(trade_day)])
