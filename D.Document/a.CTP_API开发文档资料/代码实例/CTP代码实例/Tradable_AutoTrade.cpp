...
public:
		...
		CThostFtdcRspInfoField *pRspInfo, int nRequestID,bool bIsLast)
	{
		...
		ord.OrderPriceType = THOST_FTDC_OPT_LimitPrice;  //1.限价单
		//ord.OrderPriceType = THOST_FTDC_OPT_AnyPrice;   //1.SHFE不支持市价单
		ord.Direction = THOST_FTDC_D_Buy;
		strcpy_s(ord.CombOffsetFlag, "0");
		strcpy_s(ord.CombHedgeFlag, "1");
		ord.LimitPrice = 4050;  //成交价需要根据实际调整
		ord.VolumeTotalOriginal = 1;
		ord.TimeCondition = THOST_FTDC_TC_GFD;    //2.限价单
		//ord.TimeCondition = THOST_FTDC_TC_IOC;    //2.SHFE不支持市价单
