...
for(i in seq(1,(length(trade_day)-KHolding), KHolding))
{
  if(trade_day[i]>=calendar_day[HPeriod+TPeriod])
  {
    ...
    # {
    #   list_short_csv<-c(list_csv,list(secu_Code[i]))
    # }
    # for(i in real_long_index)
    # {
...
# len_index<-length(year_index)
# 
# for(tmp_year in 1:(len_index-1))
# {
#   year2011<-bench_mark_re[year_index[tmp_year]:year_index[tmp_year+1]-1]
#   year_return<-sum(year2011,na.rm=TRUE)
#   year2011_mean<-mean(year2011,trim=0,na.rm=TRUE)
#   year2011_sd<-sd(year2011,na.rm=TRUE)
#   year2011_sharp<-year2011_mean/year2011_sd*sqrt(252)
#   
#   print(">>>>>>>>>>>")
#   print(year_index[tmp_year])
#   print(year_return)
#   print(year2011_sharp)
# }
