...
for(i in 1:length(filelist))
{
  ...
  for(j in NPeriod:length(calendar_day))
  {
    ...
    if(Inventory_Now>1e-5)
    {
      ...
      Matrix_HRatio[j,i]<-(Inventory_Now-Inventory_Last)/Inventory_Last
    }
  }
}

#============================2018.7.12 构建矩阵存储同比的环比数据==============================#
#同比的环比计算规则：从startday开始，计算上个周期的同比数据；
NPeriod<-365
Matrix_THRatio<-matrix(NA,nrow=length(calendar_day),ncol=length(secu_Code))     #同比矩阵

for(i in 1:length(filelist))
{
  #=============思路：根据上面计算的同比数据计算同比的环比数据===================#
  for(j in NPeriod:length(calendar_day))
  {
    Matrix_THRatio_Last <- Matrix_HRatio[(j-NPeriod),i]
    Matrix_THRatio_Now <- Matrix_HRatio[j,i]
    
    THRatio_Delta <- Matrix_THRatio_Now - Matrix_THRatio_Last

    Matrix_THRatio[j,i]<-THRatio_Delta
...
for(i in seq(1,(length(trade_day)-KHolding), KHolding))
{
  ...
  THRatio_Tmp<-matrix(NA,nrow =length(secu_Code),ncol = 2)
  ...
  index_tmp2<-which(Ratio_Tmp[,2]>0)
  index_tmp22<-which(Ratio_Tmp[,2]<0)
  
  #==================================================================#
  
  #==================================================================#
  THRatio_index<-which(calendar_day==trade_day[i])
  THRatio_Tmp[,1]<-(1:length(secu_Code))
  THRatio_Tmp[,2]<-Matrix_THRatio[THRatio_index,]
  index_tmp3<-which(THRatio_Tmp[,2]>0)
  index_tmp33<-which(THRatio_Tmp[,2]<0)
  #==================================================================#
  
  short_index_tmp<-intersect(index_tmp1,intersect(index_tmp2,index_tmp3))
  long_index_tmp<-intersect(index_tmp1,intersect(index_tmp22,index_tmp33))
  
  return_Short <- Matrix_Return[(i+1):(i+KHolding),short_index_tmp]
  return_Long <- Matrix_Return[(i+1):(i+KHolding),long_index_tmp]
  
  return_Short[which(is.na(return_Short)==1)] <- 0
  return_Long[which(is.na(return_Long)==1)] <- 0
  
  Total_return[(i+1):(i+KHolding)] <- rowSums(return_Long)/length(return_Long)-rowSums(return_Short)/length(return_Short)
  
  Total_return[which(is.na(Total_return)==1)] <- 0
}

#================================作图============================================#
startIndex<-0
plot(cumsum(Total_return[startIndex:length(Total_return)]),type="l")
