...
trade_day<-inventory_return[2:nrow(inventory_return)]
trade_day<-as.numeric(trade_day)
...
for(i in seq(1,(length(trade_day)-KHolding), KHolding))
{
  if(trade_day[i]>=calendar_day[HPeriod+TPeriod])
  {
    ...
    prod_mid_long <- apply(bench_re_Max_Long,2,cumprod)
    prod_mid_short <- apply(bench_re_Max_Short,2,cumprod)
    
    mid_long <- rbind(1,prod_mid_long)
    mid_short <- rbind(1,prod_mid_short) 
    
    diff_mid_long <- diff(mid_long)
    diff_mid_short <- diff(mid_short)
    
    if(ncol(bench_re_Max_Long)<1)
    {
      long_profit <- 0
    }
    else
    {
      long_profit <- rowSums(diff_mid_long)/ncol(diff_mid_long)-0.0001
    }
    
    if(ncol(bench_re_Max_Short)<1)
    {
      short_profit <- 0
    }  
    else
    {
      short_profit <- rowSums(diff_mid_short)/ncol(diff_mid_short)+0.0001
    }
    
    bench_mark_re[(i+1):(i+KHolding)] <- long_profit - short_profit
